async function fetchSwisscomOutages(npa) {
    ...
    try {
        ...
        const outagesToDisplay = data.map(outage => ({
            ID: outage.id,
            State: outage.state,
            ...
            Localisation: outage.location.substring(0, 12) + '...',
            Description_Courte: outage.description.substring(0, 12) + '...'
        }));

        // DISPLAY
        console.info(`Swisscom outages for NPA ${npa}: `);
        console.table(outagesToDisplay);
